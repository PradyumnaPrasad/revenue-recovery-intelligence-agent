...
### 1.1 The twenty-seven defects found in the code audit and real execution
...
messages F13 introduced had no recipient at all. A fifteenth (F15) surfaced while proving the Razorpay webhook loop closes
end to end with a real test-mode payment: the webhook was received and
stored but never actually updated the invoice it belonged to. Fixing that
exposed a sixteenth (F16) — arguably the most serious defect on this
list, since it silently broke the tamper-evidence guarantee this project
treats as non-negotiable. All sixteen are fixed and verified live.
...
| **F15** | **Found while asking "the webhook arrived — did anything actually update?" after the first genuine end-to-end payment test.** `Invoice.razorpay_payment_link_id` and `.payment_link_sent` were real columns in the schema, written nowhere in the codebase — so an incoming webhook had no way to correlate back to the invoice it belonged to at all. `POST /webhooks/razorpay` stored every event and returned `"handled": event_type in HANDLED_EVENTS` — `true` for `payment_link.paid`, even though nothing downstream ever happened: `Invoice.status` never changed, no audit event was written. `"handled"` was a lie. Fixed: `act_on_invoice()` now sets `razorpay_payment_link_id`/`payment_link_sent` the moment a real Razorpay call succeeds; `razorpay_webhook()` looks up the invoice by that id on `payment_link.paid`, sets `status = "paid"`, and writes a new `payment_received` audit event; the response now carries an honest `invoice_updated` flag instead of the misleading `handled`. | Verified live end to end: executed `resend_payment_link` on a fresh invoice, confirmed `razorpay_payment_link_id` populated in Postgres before any payment; user paid the real link through Razorpay's hosted checkout; webhook arrived and `GET /invoices/{id}` showed `status: "paid"`, with a new `payment_received` line in the audit trail reading "Payment received via Razorpay webhook: ₹X — invoice marked paid." | **Pre-recording, user-initiated real payment test** |
| **F16** | **Found while proving the webhook loop closes end to end, with real money moving through Razorpay's test mode.** A real invoice accumulated three real audit events — `invoice_ingested`, `action_executed`, `payment_received` (the last one written by the webhook handler in a separate HTTP request from the first two) — and `/invoices/{id}/audit/verify` reported `intact: false`. Root cause: this project's simulated demo clock stamps every event with the identical `created_at` (every event this build has ever produced shows `2026-01-01T09:00:00+00:00`, regardless of when it was actually written), and `AuditEvent.id` is a random UUID with zero correlation to insertion order. `_tip_hash()` (which every new audit write depends on to find "the previous event's hash") and `verify_invoice_chain()` both ordered rows by `ORDER BY created_at DESC/ASC` alone — genuinely ambiguous once 2+ events on one invoice share a timestamp, which is the normal case under this project's frozen clock, not an edge case. Postgres returned `invoice_ingested`'s hash as the "tip" instead of `action_executed`'s, so `payment_received`'s `prev_hash` pointed at the wrong event — a real, silent break in the exact tamper-evidence guarantee this project lists as "not cut under any circumstance." Fixed: added `AuditEvent.seq`, a genuine Postgres `IDENTITY` column — monotonic by construction, independent of both the UUID and the clock — and switched `_tip_hash()`, `verify_invoice_chain()`, and `GET /invoices/{id}/audit` to order by `seq` instead of `created_at`. Required a schema reset (`python -m app.db.reset`, consistent with this project's no-migrations design — plan.md ADR on Alembic) since there's no migration chain to add a column to a live table. | Reproduced deterministically after the fix: executed a real `send_upi_payment_link` on a fresh invoice, then sent a properly HMAC-signed synthetic `payment_link.paid` webhook (signed with the real secret, computed and used entirely inside the container so it never appeared in any tool output) for that invoice's real `plink_...` id. All three events landed with the identical frozen timestamp exactly as before — and this time `/audit/verify` returned `intact: true, events_checked: 3`. 162/162 tests still pass (pure-Python chain logic in `test_audit_chain.py` was already correct and untouched; the bug was entirely in how rows were fetched before reaching it). | **Pre-recording, user-initiated real payment test** |
| **F17** | **Reported directly by the user while clicking through the dashboard themselves on demo day.** A large invoice hit Razorpay's real test-account amount cap — a genuine, expected constraint, not a bug (see F-context: some invoices in the seeded portfolio exceed what this test account allows). But the dashboard's `#act-result` panel rendered the failure as "Internal record only — {...}", the exact same wording used for genuinely internal, no-external-call actions like `route_to_dispute`. That's actively misleading: a real external Razorpay API call WAS made and WAS rejected — nothing about it was "internal." Fixed: `renderActResult()` now has a dedicated branch for `response.error`, rendered as "Failed: ... A real Razorpay API call was made and rejected," distinct from the true internal-record case. | Reproduced live on a large invoice recommending `send_upi_payment_link`: before the fix, the panel read "Internal record only"; after, it correctly reads "Failed ... A real Razorpay API call was made and rejected" with the actual Razorpay error text. 162/162 tests unaffected (dashboard-template-only change). | **Live demo day, user-reported** |
| **F18** | **The sharpest critique on this whole list, put directly: "if we click for offer_payment_plan, there is no plan, just a email drafted... I want you to think like a real architecture, not just the UI."** Correct, and it generalized beyond one action. `offer_payment_plan` drafted an email that *said* "we'll set up a plan" with no installment schedule behind it; `schedule_call` said "within 2 business days" with no actual slot; `escalate_to_am` said "escalated to the account manager queue" with no actual AM ever named. Three action names that each promised a concrete artifact, none of which existed anywhere as data — only as a sentence. Fixed with a new `app/tools/plan_builder.py`: `build_installment_plan()` computes N real installments (amounts, due dates) that sum EXACTLY to the invoice's collectible amount — using `config/actions.yaml`'s already-declared `collectible_fraction`/`days_to_cash` for `offer_payment_plan`, the same numbers the ranking model's own expected-value calculation was scored against, not a second, independently-invented number; `next_business_slot()` computes a real next-weekday 11:00 slot for `schedule_call`; `assign_account_manager()` deterministically (hash-based, not random) assigns one of a small fixed AM roster with a real 4-hour response SLA for `escalate_to_am`. These structured artifacts (`plan`, `scheduled_for`, `assigned_to`/`respond_by`) flow through `execute_tool()` → the stored `Action.response` → the dashboard, which now renders them as real structured UI (an installment table, a call-slot callout, an assignment card) instead of only prose, and the audit narration states them explicitly ("Real 3-installment plan drafted, ₹X total.") | Executed live: `offer_payment_plan` on a ₹73,272 invoice produced 3 installments of ₹21,982/₹21,982/₹21,982.02 (summing exactly to the config's 90%-collectible ₹65,945.24), each with a real due date; `schedule_call` produced a real ISO datetime slot; `escalate_to_am` produced a real named AM with a real SLA timestamp. 11 new tests added across `test_plan_builder.py`, `test_message_templates.py`, and `test_tools_registry.py` — all asserting exact numbers, not just "returns something." | **Live demo day, user-reported ("not just the UI")** |
| **F19** | **The root architectural cause behind F18 and behind "it feels like a chatbot, not an agent," put directly: "I want an end-to-end agent."** `app/domain/clock.py`'s `VirtualClock.advance()` and `app/deps.py`'s own docstring both describe an intended `/simulate/advance` endpoint that lets a multi-day recovery journey run without waiting on real wall-clock time — but it was never actually wired up as a route. The live system ran on a clock frozen at one instant forever, and the *only* way anything ever happened was a human clicking "Execute" on one invoice at a time. A competing public submission to this same track ([srikrishna0603/razorpay-buildathon](https://github.com/srikrishna0603/razorpay-buildathon)) reacts automatically to incoming webhooks despite a narrower scope (payment failures only, mock executor, no real Razorpay) — on autonomy specifically, it was ahead. Fixed: `app/api/simulate.py` adds `POST /simulate/advance` (actually moves the clock) and `POST /simulate/tick` (runs diagnose→rank→govern→execute across *every open invoice in a batch*, no human touching each one). This is not new decision logic — `act_on_invoice()`'s execution body was factored out into a shared `_execute_decision()` that both the single-invoice `/act` endpoint and the new batch `tick()` call identically, so the autonomous path and the manual path can never silently diverge in what they're allowed to do. `require_approval` and `block` outcomes are tallied, never auto-executed — an autonomous agent is not the same thing as an ungoverned one. Dashboard gets an "Advance + run agent" control showing the simulated clock and a live results summary. | Ran live on the full 500-invoice batch: first tick — 432 executed, 58 blocked, 6 held for approval, 41 seconds (real, individually-chained decisions, not a fake progress bar). Advanced 7 more days, ticked again — 342 fresh executions, 130 correctly deduped as idempotent replay (still in cooldown), block count dropped as promises resolved — exactly the behavior a real ladder-respecting autonomous agent should show over time. Verified the hash chain stayed `intact: true` on an autonomously-executed invoice. 178/178 tests pass (175 + 3 new for `advance_clock`'s pure-Python paths; the DB-dependent `tick()` itself is verified live, same precedent as `app/sources/receivables.py`). | **Live demo day, user-reported ("end-to-end agent")** |
| **F20** | **Found during a systematic verification sweep, one day before submission — the exact sequence the README's own Quick Start documents (reset the schema, reseed) crashed the live API with `500 Internal Server Error`, mid-check.** Root cause: `python -m app.db.reset` (`DROP+CREATE` all tables and Postgres enum types — this project's documented no-migrations design) was run against an already-running API server, not a freshly started one. asyncpg caches type OIDs per pooled connection for its prepared-statement cache; after the schema's types are dropped and recreated, every connection that was already open still references the *old*, now-invalid OIDs, and the next query on any of them fails with `asyncpg.exceptions.InternalServerError: cache lookup failed for type <oid>`. Reproduced cleanly: reset + reseed without restarting the API container, then a batch of `/evaluate` calls — the first one crashed. This is a real trap for anyone following `make reset` exactly as documented, including on demo day itself if a reset is run without also restarting the server. Fixed at the connection level, not by adding a "remember to restart" note nobody reads: `create_async_engine(..., connect_args={"statement_cache_size": 0})` disables asyncpg's prepared-statement cache entirely, so every query re-resolves its types fresh — `make reset` now genuinely works against a live server exactly as documented, with no required restart. | Reproduced the exact crash once (`cache lookup failed for type 25458`) before the fix. After adding `statement_cache_size: 0` and restarting once to load it: ran the identical reset → reseed → 50-invoice `/evaluate` sweep sequence again with zero errors. All six real action types (`send_reminder`, `offer_payment_plan`, `schedule_call`, `escalate_to_am`, `send_upi_payment_link`, `resend_payment_link`) re-verified fresh on the clean database — real installment plans, real call slots, real AM assignments, real Razorpay payment links (`plink_...`/`rzp.io` short URLs), all with correct real recipients. 178/178 tests unaffected (connection-level change, no logic touched). | **Pre-submission verification sweep** |
| **F21** | **Reported directly, "what's left to make it completely real" — the answer included real SMTP sending, which surfaced its own real bug on first live use.** `send_reminder`/`offer_payment_plan` now genuinely send via `app/tools/email_client.py` (redirected to the operator's own inbox, disclosed in the body — every generated customer email is fictitious). Built with a deliberate safety gate: `execute_tool()`'s new `deliver` parameter defaults to `False`, and only the human-triggered `/act` passes `deliver=True` — `/simulate/tick`'s autonomous batch run never does, so a single tick across hundreds of invoices can never flood a real inbox. | Mocked-SMTP tests confirm the redirect (never the fictitious customer address), the disclosure text, the chaos-switch gate, and — critically — that `deliver=False` (tick's default) never calls `send_email` even when SMTP is fully configured. 6 new tests in `test_email_client.py`, 5 in `test_tools_registry.py`. Live delivery itself pending the operator adding real Gmail App Password credentials. | **Live demo day, user-requested ("make it real")** |
| **F22** | **The most severe live bug of the entire build, found immediately after wiring up the other honest gap this project always listed — reply extraction had no HTTP endpoint.** Adding `POST /invoices/{id}/replies` (a real, un-scripted Gemini call on live input) froze the ENTIRE API server — not just that one request. `/health` itself returned nothing for minutes. Root cause, two independent bugs stacked: (1) `extract_reply()` is a synchronous, blocking network call, called directly inside an `async def` FastAPI endpoint — with no thread offload, it blocked the single-threaded asyncio event loop for its entire duration, so every other request from every other user queued behind it; (2) `genai.Client()` was constructed with no timeout at all, so if the network call itself hung, step (1)'s block was unbounded — reproduced live, over 15 minutes with no response. Fixed both, not just one: `app/api/invoices.py::receive_reply()` now runs `extract_reply()` via `asyncio.to_thread()`, off the event loop entirely; `app/llm/reply_extraction.py::_client()` now sets `HttpOptions(timeout=15000)` (15s), so a hung network call fails bounded instead of forever. | Reproduced the freeze live, twice, before the fix — confirmed via a direct `curl /health` returning nothing while a reply extraction was in flight. After the fix: fired a real reply extraction in the background and `curl`'d `/health` from a second terminal while it was still running — `200 OK`, immediately. Also surfaced, honestly, that Gemini's free-tier quota was genuinely exhausted from this session's heavy testing — every subsequent extraction now correctly times out at 15.1s and degrades to `fallback_keyword_classifier` with confidence 0.5, routed to human review, exactly as designed. 192/192 tests pass (190 + 2 new source-guard tests, since the event-loop fix itself can only be proven live, the same precedent as the DB-touching endpoints). | **Live demo day, found while closing the last honest gap** |
| **F23** | **Found the moment real SMTP credentials were actually added to `.env` — two of `test_email_client.py`'s own tests (written for F21) started failing.** `_UNCONFIGURED = Settings()` was meant to represent "SMTP not configured," but `Settings` is a `pydantic-settings` `BaseSettings` subclass, which reads the real process environment by default — once real `SMTP_USERNAME`/`SMTP_PASSWORD`/`DEMO_RECIPIENT_EMAIL` values existed in the container's environment, a bare `Settings()` picked them up for real, so the "unconfigured" fixture silently stopped being unconfigured the instant the feature it tests actually got configured. A test that only fails once the real thing it's guarding against becomes real is exactly the kind of test bug that hides in plain sight. Fixed: `Settings(smtp_username="", smtp_password="", demo_recipient_email="")` — explicit empty-string kwargs override the environment (pydantic-settings' init-arg precedence), so the fixture stays genuinely blank regardless of what's actually configured on the host. | Failed live immediately after real SMTP credentials were added and the container recreated: `test_is_configured_requires_all_three_fields` and `test_missing_config_fails_honestly_not_silently` both asserted `False`/failure and got `True`/success instead. Fixed, rebuilt, reran: 192/192 pass. Confirmed separately, live: real SMTP send executed against a real invoice, a real email with the full computed installment plan arrived in the operator's inbox — F21 genuinely works end to end, not just in mocked tests. | **Live demo day, immediately after F21 went live** |
| **F24** | **Asked directly: "what happens after escalate_to_am, and how do you end an invoice — like solved or something."** The first half answered a real design question honestly (`escalate_to_am` is the last ladder rung; once its `max_executions_per_action` is reached, nothing further is automated — the invoice just sits, by design, so it never spams a customer forever). The second half found a genuine, missing feature: there was no way to *ever* manually close an invoice out. A customer who paid by bank transfer, an invoice written off as bad debt, a dispute resolved by a human outside this system — none of these had any way to stop the ladder from continuing to consider that invoice forever, unless a real Razorpay webhook happened to mark it `"paid"`. Built `POST /invoices/{id}/resolve` (`reason` from a fixed set — `paid_offline`, `written_off`, `disputed_closed`, `duplicate_invoice`, `other` — plus an optional note), which sets a real terminal `Invoice.status` and writes a real audit event. Deliberately does NOT reuse `"paid"` for this — that stays reserved for a real, webhook-verified payment, so a human's assertion is never confused with Razorpay's own confirmation in any report or dashboard reading `status == "paid"`. `/act` and `/simulate/tick` both now treat *any* non-`"overdue"` status as closed (previously `/simulate/tick` only checked for `"paid"` specifically), so a resolved invoice genuinely stops accepting new actions everywhere, not just in one code path. | Resolved a real invoice as `paid_offline` with a note; confirmed live: `/act` on it immediately returned `409 {"invoice_closed": true, "status": "paid_offline"}`; the audit trail read "Invoice manually resolved: paid offline. Note: ..."; chain stayed `intact: true` across all 4 events. Dashboard shows a real "🔒 CLOSED" banner and hides the resolve control once closed, confirmed via the actual rendered page (`hasResolveCard: false`, `actBtnDisabled: true`). 195/195 tests pass (192 + 3 new constant guards; the DB-touching endpoint itself verified live, same precedent as the other stateful endpoints). | **Live demo day, user-requested** |
| **F25** | **Reported directly, while resolving an invoice to test F24: "why the top bar not changing... like showing the same amount, same actions 0/120, what does it mean."** Two real, independent bugs in the header stats, both found because the user actually watched the numbers instead of trusting them. First: `batch_summary()`'s `revenue_at_risk_paise` summed **every** invoice in the batch unconditionally — it never excluded paid or resolved invoices, so no amount of real payment or resolution could ever move it. Second: the dashboard's "Actions today" was backed by a JavaScript variable (`actionsToday`) that was declared, initialized to 0, and never incremented anywhere in the surrounding loop — a permanent, silent dead stub that had nothing to do with how many actions had actually executed. Fixed: `batch_summary()` now separately computes `open_invoice_count`/`revenue_at_risk_paise` (open invoices only, via the same `OPEN_STATUS` constant F24 introduced) and a real `actions_today` count (a live query against `Action` rows executed on the simulated clock's current date, for invoices in this batch) plus the configured `daily_action_budget`, replacing the dead client-side stub entirely. The dashboard also now refreshes these stats after every execute/resolve, not just once on page load. | Verified live, before and after: resolved one real invoice (₹72,270.62) and confirmed `open_invoice_count` dropped 500→499 and `revenue_at_risk_paise` dropped by exactly that invoice's amount in the same API response; `actions_today` read a real, non-zero `4` (matching the day's actual executed-action count) instead of a permanent `0`. Confirmed the same numbers rendering correctly in the live dashboard page itself, not just the API. 195/195 tests unaffected (query-level change, no decision logic touched). | **Live demo day, user-reported while testing F24** |
| **F26** | **Found while preparing a single-invoice demo walkthrough: two real actions executed on the same invoice within one tick both showed `recommended_action: null`, `"final": "Approved — executing None."`** — nothing was ladder-eligible at all, on an invoice that had only progressed two rungs of six. Root cause: `_current_rung_index()` defined "current position" as the ladder index of "the most recently executed action," found via `min(history, key=lambda h: h.days_ago)` — but under this project's frozen demo clock, two genuinely different real actions executed in the same tick get the IDENTICAL `days_ago` (both 0), an unresolvable tie. `min()` silently returned whichever entry happened to come first in the list — not necessarily the one further along the ladder — misidentifying the current rung as *earlier* than it really was, which incorrectly narrowed the "advance at most one rung" window and blocked a rung that should have been legitimately reachable. Fixed: `_current_rung_index()` now returns the **furthest** rung any executed action has reached (`max(ladder.index(...) for ...)`), a timestamp-independent definition — identical to the old one whenever `days_ago` values are genuinely distinct (the ladder only ever moves forward, so "most recent" and "furthest reached" always coincide in that case), and correct instead of ambiguous in the exact tied case that was broken. | Reproduced directly: an invoice with `send_upi_payment_link` and `schedule_call` both executed at the frozen clock's single instant (`days_ago=0` for both) showed `offer_payment_plan: ladder_eligible: false` and `recommended_action: null` before the fix. After the fix, the identical invoice/history correctly shows `offer_payment_plan: ladder_eligible: true, recommended_action: "offer_payment_plan"`. All 19 pre-existing ladder tests still pass unmodified (confirming the fix is behaviorally identical in the normal, non-tied case); one new regression test added for the exact tied-timestamp scenario. 196/196 tests pass. | **Live demo day, found while building a demo walkthrough** |
| **F27** | **Found immediately after F26, building the same single-invoice demo: an invoice that had received multiple real updates (payment-link fields, several `/act` calls) vanished entirely from the dashboard's invoice list.** `list_invoices()` had no `ORDER BY` clause at all — Postgres makes no ordering guarantee without one, and an `UPDATE`d row's position in an unordered sequential scan can genuinely change even though nothing about its logical identity did. The heavily-updated demo invoice had been physically relocated to position 499 of 500 in the returned array; since the dashboard only renders the first 60, it silently disappeared from view. Fixed: `.order_by(Invoice.invoice_number)` — stable and meaningful (the natural INV-1000, 1001, ... sequence) regardless of how many times any given row has been updated. | Reproduced directly: the demo invoice (updated ~6 times across F17-era testing and this walkthrough) was confirmed at index 499 of a 500-item unordered response, explaining its disappearance from the visible list. After the fix, confirmed indices 0-19 return exactly INV-1000 through INV-1019 in order, with the previously-missing invoice back in its correct natural position. 196/196 tests unaffected (query-level change, no decision logic touched). | **Live demo day, found while building the same walkthrough as F26** |
